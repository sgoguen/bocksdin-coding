# THIS IS A BINARY ENCODING EXAMPLE WITH A SINGLE CROSSOVER
...
# length is the amount of things
def generate_genome(length: int) -> Genome:
  return choices([0, 1], k=length)

# create genomes for a specified population size
def generate_population(size: int, genome_length: int) -> Population:
  return [generate_genome(genome_length) for _ in range(size)]

# calculate weight of each genome
...
def genome_to_things(genome: Genome, things: List[Thing]) -> List[Thing]:
  ...
  for i, thing in enumerate(things):
    if genome[i] == 1:
      result += [thing.name, thing.weight]
